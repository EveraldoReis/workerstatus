...
{assign details $this->form->getFieldset('details')}
<form enctype="multipart/form-data" action="{JRoute::_("index.php?option=com_workerstatus&layout=edit&id={$this->person->id}")}"
    ...
    <div class="width-60 fltlft">
        <fieldset class="adminform">
            <legend>{JText::_('COM_WORKERSTATUS_WORKERSTATUS_DETAILS')}</legend>
            <ul class="adminformlist">
                {foreach from=$details item=field}
                    <li>
                        {$field->label}
                        {$field->input}
                    </li>
    ...
    <div class="width-40 fltrt">
        {JHtml::_('sliders.start', 'workerstatus-slider')}
    ...
    <div>
        <input type="hidden" name="task" value="workerstatus.edit" />
        {JHtml::_('form.token')}
    </div>
</form>
