...
class JFormFieldPrice extends JFormField
{
    ...
    function getInput($person = null)
    {
        require 'size.php';

        $values             = $person->complex_price;
        ...
        $type               = $input->get('price_type') ? $input->get('price_type', 0) : JFactory::getApplication()->getUserState('com_workerstatus.price_type',
                        0);
        if (!$type && is_object($person))
            $type               = $person->price_type;
        $sizeText           = JText::_('COM_WORKERSTATUS_GENERIC_HEADING_SIZE');
        $valueText          = JText::_('COM_WORKERSTATUS_GENERIC_HEADING_VALUE');
        $addText            = JText::_('COM_WORKERSTATUS_GENERIC_ADD_VARIATION');
        $removeText         = JText::_('COM_WORKERSTATUS_GENERIC_REMOVE');
        $alterPriceTypeText = JText::_('COM_WORKERSTATUS_GENERIC_ALTER_PRICE_TYPE');
        $multiplePriceText  = JText::_('COM_WORKERSTATUS_GENERIC_MULTIPLE_PRICE_TYPE');
        $singlePriceText    = JText::_('COM_WORKERSTATUS_GENERIC_SINGLE_PRICE_TYPE');
        if ($type === 'multiple_price')
        {
            ...
            $output .= '<div class="price">'
                    ...
                    . '<div class="rpt" style="display: inline-block;"><br/><button onclick="Joomla.submitbutton(\'person.rpt\');">' . $alterPriceTypeText . '</button></div>'
                    . '</div>';
            if ($values)
            {
        ...
        else if ($type === 'single_price')
        {
            ...
            $output .= '<div class="price">'
                    ...
                    . '<div style="display: inline-block;"><button onclick="Joomla.submitbutton(\'person.rpt\');">' . $alterPriceTypeText . '</button></div>'
                    . '</div>';
        }
        else
        {
            $doc->addStyleDeclaration('#price_type{padding: 5px 0 0 0;} #price_type > a{padding: 2px 10px; border: 1px solid #aaa;}');
            $output .= '<div id="price_type"><a onclick="if(document.adminForm.jform_name.value != \'\'){event.preventDefault();var input = document.createElement(\'input\');input.name = \'jform[price_type]\';input.value=\'single_price\';input.type=\'hidden\';document.adminForm.appendChild(input);Joomla.submitbutton(\'person.reload\');}" class="btn" href="' . JRoute::_('index.php?option=com_workerstatus&view=person&layout=edit&id=' . $input->get('id') . '&price_type=single_price') . '">' . $singlePriceText . '</a>&nbsp;&nbsp;&nbsp;&nbsp;<a onclick="if(document.adminForm.jform_name.value != \'\'){event.preventDefault();var input = document.createElement(\'input\');input.name = \'jform[price_type]\';input.value=\'multiple_price\';input.type=\'hidden\';document.adminForm.appendChild(input);Joomla.submitbutton(\'person.reload\');}" class="btn" href="' . JRoute::_('index.php?option=com_workerstatus&view=person&layout=edit&id=' . $input->get('id') . '&price_type=multiple_price') . '">' . $multiplePriceText . '</a></div>';
        }
        return "$output</div>";
    }
